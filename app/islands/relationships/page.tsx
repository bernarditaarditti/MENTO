...
export default function RelationshipsIslandPage() {
  ...
  const getInitialLevel = (): number => {
    ...
    const level1Completed = localStorage.getItem("relationships_level_1_completed")
    const level2Completed = localStorage.getItem("relationships_level_2_completed")
    const level3Completed = localStorage.getItem("relationships_level_3_completed")
    const level4Completed = localStorage.getItem("relationships_level_4_completed")
  ...
  const getInitialCompletedLevels = (): Set<number> => {
    ...
    if (localStorage.getItem("relationships_level_1_completed") === "true") completed.add(1)
    if (localStorage.getItem("relationships_level_2_completed") === "true") completed.add(2)
    if (localStorage.getItem("relationships_level_3_completed") === "true") completed.add(3)
    if (localStorage.getItem("relationships_level_4_completed") === "true") completed.add(4)
        ...
        {Array.from({ length: totalLevels }, (_, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg select-none ${level > currentLevel ? 'level-locked-relationships' : ''}`}
                style={{
                  backgroundColor: level > currentLevel ? "#FFAC88" : getLevelColor(level),
                  boxShadow: level === 1 
                ...
                onClick={(e) => {
                  if (level > currentLevel) {
                    ...
                    router.push(`/islands/relationships/level-${level}`)
                  }
                }}
              >
                {level}
